# demo-stablecode-instruct-alpha-3b-gptq
![](https://www.banana.dev/lib_zOkYpJoyYVcAamDf/x2p804nk9qvjb1vg.svg?w=340 "Banana.dev")

# Banana.dev stablecode-instruct-alpha-3b-GPTQ starter template

This is a stablecode-instruct-alpha-3b-GPTQ starter template from [Banana.dev](https://www.banana.dev) that allows on-demand serverless GPU inference.
...
1. [Fork this](https://github.com/bananaml/demo-stablecode-instruct-alpha-3b-gptq/fork) repository to your own Github account.
...
Build the model as a Docker image. You can change the `stablecode-instruct-alpha-3b-gptq` part to anything.

```sh
docker build -t stablecode-instruct-alpha-3b-gptq .
```

Run the Potassium server

```sh
docker run --publish 8000:8000 -it stablecode-instruct-alpha-3b-gptq
```
